...
public sealed class LocalMcpHttpServerService
{
    private const string ServerInstructions = """
        ...
        Suggested workflow for small or local models: 1) call get_app_settings and get_regex_profiles before changing anything; 2) use test_regex_chain to verify complex regular-expression chains against sample text; 3) for one-rule edits prefer insert_regex_chain_item, update_regex_chain_item, or delete_regex_chain_item so existing rules shift automatically instead of being rebuilt from memory; 4) call upsert_regex_profile only when creating a new profile or replacing a full profile; 5) OCR rule: call get_ocr_languages, copy an exact languageTag, then call the item's useTool (set_ocr_settings, install_ocr_language, or none) to use that language; call deleteTool only when it equals delete_ocr_language; never delete the current OCR language; 6) change McpPort only while McpEnabled is false; 7) setting McpEnabled to false immediately disables external MCP access. Use standard MCP tool descriptions, inputSchema descriptions, and tool annotations as the authoritative guide.
        """;
